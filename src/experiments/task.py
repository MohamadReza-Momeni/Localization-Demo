...
class SimulationTask:
    ...
    def execute(self, run_id: int) -> list[dict]:
        ...
        # Serialize the actual anchor positions used in this run. Anchors are now
        # generated with a random seed (not derived from run_id), so they can no
        # longer be reproduced later just by re-seeding — we must persist the exact
        # coordinates so map_generator.py can plot the anchors that were really used.
    ...
    def _generate_scenario(self, run_id: int):
        # UPDATED: anchors and targets both now use genuinely random seeds
        # (seed=None -> OS entropy) instead of fixed seeds derived from run_id.
        # This means every run gets a different anchor layout AND a different
        # target position, even if you re-run main.py with the same run_id.
        # This is intentional and safe under ProcessPoolExecutor: numpy's default_rng(None)
        # pulls fresh OS entropy per process/call, so worker processes won't collide.
        # NOTE: target positions don't need to be persisted separately like anchors —
        # true_x/true_y are already saved per result row in results.csv, so
        # map_generator.py needs no changes; it already reads targets from there.
